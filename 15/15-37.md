...
class NotQuery : public Query_base{
    friend Query operator~(const Query &);
    NotQuery(shared_ptr<Query_base> q) : query(q) {cout << "NotQuery::NotQuery(const string &)" << endl;};
...
protected:
    BinaryQuery(shared_ptr<Query_base> l, shared_ptr<Query_base> r, const string &s) : lhs(l), rhs(r), opSym(s) {
    ...
    shared_ptr<Query_base> lhs, rhs;     //左侧和右侧运算对象
    string opSym;       //运算符的名字
};

class AndQuery : public BinaryQuery{
    friend Query operator&(const Query &, const Query &);
    AndQuery(shared_ptr<Query_base> left, shared_ptr<Query_base> right) : BinaryQuery(left, right, "&") {
...
Query operator&(const Query &lhs, const Query &rhs){
    return shared_ptr<Query_base>(new AndQuery(lhs.q, rhs.q));
}

class OrQuery : public BinaryQuery{
    friend Query operator|(const Query &, const Query &);
    OrQuery(shared_ptr<Query_base> left, shared_ptr<Query_base> right) : BinaryQuery(left, right, "|") {
