...
class Matches {
    ...
    friend vector<Matches> findBook(const vector<vector<Sales_data>> &, const string &);
    friend void reportResults(istream &, ostream &, const vector<vector<Sales_data>> &);
public:
    Matches(vector<Sales_data>::size_type a,
             vector<Sales_data>::const_iterator b,
             vector<Sales_data>::const_iterator c) : index(a), bg(b), ed(c) {}
private:
    ...
    while (in >> s){
        ...
        for (const auto &store : trans)     //对每家销售了给定书籍的书店
            ...
            os << "store" << (store.index + 1) << " sales: "
               << accumulate(store.bg, store.ed, Sales_data(s))
               << endl;
    }
}
